...
\begin{equation}
    ...
    \text{З}_{\text{O}} = \text{К}_{\text{пр}} \sum_{i = 1}^{n} \text{З}_{\text{ч.i}} \times t_i \text{\,,}
\end{equation}
\begin{explanationx}
...
Расчет дополнительных выплат, предусмотренных законодательством о труде, осуществляем по формуле 
...
\FPeval{\additionalSalary}{trunc(\commonMainSalary * \additionalSalaryPercents / 100, 2)}

\begin{equation}
    \label{sec_econom:eq:main_salary_count}
    \text{З}_{\text{Д}} = \frac{\commonMainSalary \times \additionalSalaryPercents}{100\%} = \additionalSalary \text{\,,}
\end{equation}
